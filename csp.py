...
class CSP(search.Problem):
    ...
    def __init__(self, variables, domains, neighbors, constraints, weights=None):
        ...
        self.nassigns = 0
        self.constraint_check_count = 0  # Counter for constraint evaluations
        self.weights = weights if weights is not None else {}  # Add weights as an attribute
        
    def update_weight(self, var1, var2, weight):
        """Update the weight for a specific constraint between two variables."""
        if (var1, var2) in self.weights:
            self.weights[(var1, var2)] = weight + 1 
        elif (var2, var1) in self.weights:
            self.weights[(var2, var1)] = weight + 1
        else:
            # Add the weight if it doesn't exist yet
            self.weights[(var1, var2)] = weight + 1
    ...
    def nconflicts(self, var, val, assignment):
        ...
        def conflict(var2):
            if var2 in assignment:
                self.constraint_check_count += 1  # Increment the counter for each constraint check
                return not self.constraints(var, val, var2, assignment[var2])
            return False
...
def dom_wdeg(assignment, csp):
    # Select the variable based on the domain/degree ratio
    return argmin_random_tie([v for v in csp.variables if v not in assignment],
                             key=lambda var: domain_weight_ratio(csp, var, assignment))


def domain_weight_ratio(csp, var, assignment):
    """Calculates the domain/degree ratio for a given variable, considering constraint weights."""
    if csp.curr_domains:
        # Calculate the weighted degree for the variable
        weighted_degree = 0
        for neighbor in csp.neighbors.get(var, []):
            if neighbor not in assignment:
                # Check if a constraint exists and add its weight if so
                if (var, neighbor) in csp.weights:
                    weighted_degree += csp.weights[(var, neighbor)]
                elif (neighbor, var) in csp.weights:
                    weighted_degree += csp.weights[(neighbor, var)]

        # If there is no weighted degree, return a high value to indicate high priority
        if weighted_degree == 0:
            return sys.maxsize
        
        # Return the ratio of the size of the domain to the weighted degree
        return len(csp.curr_domains[var]) / weighted_degree
    else:
        # Fallback for CSP without `curr_domains`: check the count of valid values
        return sum(csp.nconflicts(var, val, assignment) == 0 for val in csp.domains[var])
